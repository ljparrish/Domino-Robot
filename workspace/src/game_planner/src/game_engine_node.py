...
def game_engine():
    ...
    while not turn_over: 
        ...
        board_dom = np.vstack((board_dots_half1,board_dots_half2))
        # Initializes positions of board dominoes on computer's grid
        board_pos = np.array([[4,3,2,0],
                              [3,2,1,1],
                              [4,5,2,1],
                              [4,2,2,1]])
        board_dom_orientation = np.array(board_dom_orientation)

        for i in range(np.size(board_dom,1)):
            place_domino(board, board_dom[:,i], board_pos[0,i], board_pos[1,i], board_dom_orientation[i])
